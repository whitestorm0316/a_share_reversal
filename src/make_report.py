...
A('<div class="meta">标的池 4,986 只（主板/创业板/科创板，已剔北交所）· '
  ...
  '复权：前复权(hfq) · 成本模型：佣金万2.5 + 过户费 + 印花税(2023-08-28 起 0.05%) '
  '+ 双边滑点</div>')
A("</div></header>")
A('<div class="wrap">')
...
for k, v in [
    ...
    ("价格数据", "腾讯行情前复权(hfq)日线，2015-12-01 起，"
...
A('<h3>1.3 关于「为什么用前复权」</h3>')
A('<p>前复权价在长期历史段可能为负或极小，会扭曲百分比止损。'
  '本引擎在入场前会检查 <code>开盘价 &gt; 0</code> 且 <code>前收盘 &gt; 0</code>，'
  '不满足则丢弃该信号。同时用「前收盘」而非「复权价绝对值」做涨跌幅判断，'
  '确保涨跌停判定不受复权系数影响。</p>')
...
A('<div class="callout warn"><span class="h">原策略的「盈亏平衡滑点」大约只有 6bp。</span>'
  f'毛收益 {P(B["avg_gross"] if "avg_gross" in B else e11.iloc[0]["avg_gross"])}，'
  '这意味着只要双边滑点合计超过约 6bp，策略就变负。'
...
bc = f1[f1.key == "ruleC"].iloc[0]
for k, v, note in [
    ("信号定义", "超跌D2-D6 + 2日不创新低 + 站上MA5", "无成交量条件"),
    ("样本数", f"{int(bc['n']):,} 笔", "2016-04 ~ 2026-09 全市场"),
    ("单笔净收益", P(bc["avg_ret"]), f"t = {T(bc['t_stat'])}，显著为正"),
    ("胜率", f"{bc['win_rate']*100:.1f}%", "低于 50%，靠右尾盈利"),
    ("平均持有", f"{bc['avg_hold']:.1f} 个交易日", "—"),
    ("样本内（16-20）", P(qrow(f2, "cand", "规则C(缩量A)", "period", "样本内16-20")["avg_ret"]),
     "含缩量A的版本，作为参照"),
    ("样本外（21-26）", P(qrow(f2, "cand", "规则C(缩量A)", "period", "样本外21-26")["avg_ret"]),
     "含缩量A的版本，作为参照"),
    ("滑点 20bp 时", P(qrow(f3, "cand", "规则C(超跌+缩量A+不创新低+站上MA5)", "slippage", 0.002)["avg_ret"]),
     "优势大幅缩水"),
    ("滑点 30bp 时", P(qrow(f3, "cand", "规则C(超跌+缩量A+不创新低+站上MA5)", "slippage", 0.003)["avg_ret"]),
     "转负"),
    ("牛市条件下", P(qrow(f4, "regime", "牛市")["avg_ret"]), "显著为负，应停用"),
    ("熊市条件下", P(qrow(f4, "regime", "熊市")["avg_ret"]), "显著为正，最佳环境"),
    ("理论净值 CAGR", H.pct(CV["ruleC"]["cagr"]), "等权篮子口径，非实盘"),
    ("最大回撤", H.pct(CV["ruleC"]["max_dd"]), "—"),
]:
...
A('<p>「参数稳定」意味着策略表现对参数的小幅变动不敏感。'
  '下表是对最关键的三个参数做邻域检验的结果（以规则B/C 为基准）：</p>')
...
A('<div class="callout bad"><span class="h">但我必须再强调一次：稳定 ≠ 值得做。</span>'
  ...
  f'{P(qrow(f3,'cand','规则C(超跌+缩量A+不创新低+站上MA5)','slippage',0.002)['avg_ret'])}，'
  '在 30bp 下为负。'
...
QA = [
    ...
    ("Q10", "这个策略是否有统计优势？是否可以实盘使用？", "no",
     ...
     "② 滑点从 10bp 到 20bp，优势缩水过半；到 30bp 转负；"
...
A('<pre>'
  ...
  '  data/daily.parquet        5,187 只 × 1,146 万行前复权日线（2015-12-01 起）\n'
